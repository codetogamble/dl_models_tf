...
target_image = tf.placeholder(tf.float32,shape=[1,image_size,image_size],name="target_image")
...
conv1 = Layers.Conv2D(3,1,1,padding="same",activation='relu',use_bias=True,kernel_constraint=kernel_constr,bias_constraint=bias_constr)(input_image_reshaped)

# conv1 = tf.layers.batch_normalization(conv1, training=True)
conv2 = Layers.Conv2D(5,3,2,padding="same",activation='relu',use_bias=True,kernel_constraint=kernel_constr,bias_constraint=bias_constr)(conv1)
# conv2 = tf.layers.batch_normalization(conv2, training=True)
conv3 = Layers.Conv2D(7,3,2,padding="same",activation='relu',use_bias=True,kernel_constraint=kernel_constr,bias_constraint=bias_constr)(conv2)
conv3 = tf.layers.batch_normalization(conv3, training=False)
conv4 = Layers.Conv2D(9,5,2,padding="same",activation='relu',use_bias=True,kernel_constraint=kernel_constr,bias_constraint=bias_constr)(conv3)
# conv4 = tf.layers.batch_normalization(conv4, training=True)
conv5 = Layers.Conv2D(12,5,2,padding="same",activation='relu',use_bias=True,kernel_constraint=kernel_constr,bias_constraint=bias_constr)(conv4)
# conv5 = tf.layers.batch_normalization(conv5, training=True)
conv6 = Layers.Conv2D(16,5,2,padding="same",activation='relu',use_bias=True,kernel_constraint=kernel_constr,bias_constraint=bias_constr)(conv5)
...
# print(num_features)
fc1 = Layers.Dense(512,activation='relu')(lflat)
...
error = tf.math.square(tf.subtract(deconv6,target_image))
...
for f in os.listdir(test_folder):
	...
	while(tt):
		...
		if len(img_array) == batch_size+1 and len(img_array2) == batch_size+1:
			...
			# print(c.shape)
			# exit()
			c = sess.run(cost,feed_dict={input_image:img_array[:-1],target_image:img_array[-1:]})
			print(c)
			c = sess.run(cost,feed_dict={input_image:img_array2[:-1],target_image:img_array2[-1:]})
			print(c)
			for ii in range(1):
				sess.run(optimizer,feed_dict={input_image:img_array[:-1],target_image:img_array[-1:]})
				sess.run(optimizer,feed_dict={input_image:img_array2[:-1],target_image:img_array2[-1:]})
