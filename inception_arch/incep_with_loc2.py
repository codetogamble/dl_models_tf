...
input_target = tf.placeholder(tf.float32,shape=[None,target_img_size,target_img_size,3],name="input_images_array")

conv1 = Layers.Conv2D(3,1,1,padding="same",activation='relu')(input_image_reshaped)
conv2 = Layers.Conv2D(5,3,2,padding="same",activation='relu')(conv1)
conv3 = Layers.Conv2D(5,5,1,padding="same",activation='relu')(conv2)
conv3 = tf.nn.max_pool(value=conv3,ksize=[1, 2, 2, 1],strides=[1, 2, 2, 1],padding='SAME')
conv4 = Layers.Conv2D(7,5,2,padding="same",activation='relu')(conv3)
conv4 = tf.nn.max_pool(value=conv4,ksize=[1, 2, 2, 1],strides=[1, 2, 2, 1],padding='SAME')
conv5 = Layers.Conv2D(7,7,2,padding="same",activation='relu')(conv4)
deconv1 = Layers.Conv2DTranspose(5,5,2,padding="same",activation='relu')(conv5)
deconv2 = Layers.Conv2DTranspose(3,7,2,padding="same",activation='relu')(deconv1)
error = tf.math.square(tf.subtract(deconv2,input_target))
...
for epoch in range(20):
	for x in training_set[:]:
		...
		fem_loc = np.zeros((target_img_size,target_img_size),dtype=np.float32)
		pel_loc = np.zeros((target_img_size,target_img_size),dtype=np.float32)
		gen_loc = np.zeros((target_img_size,target_img_size),dtype=np.float32)
		fem_locf = np.zeros((target_img_size,target_img_size),dtype=np.float32)
		pel_locf = np.zeros((target_img_size,target_img_size),dtype=np.float32)
		gen_locf = np.zeros((target_img_size,target_img_size),dtype=np.float32)
		# print(x[0])
		if x[0] in fem_list:
			# print("FOUND")
			fem_tag_img = cv2.imread(tag_dir_pathfem+x[0],0)
			ffpts,ffptsf = getFlippedZoomImages(fem_tag_img)
			fem_loc = cv2.resize(ffpts/255,(target_img_size,target_img_size))
			fem_locf = cv2.resize(ffptsf/255,(target_img_size,target_img_size))
		...
		if x[0] in pel_list:
			...
			pel_loc = cv2.resize(pppts/255,(target_img_size,target_img_size))
			pel_locf = cv2.resize(ppptsf/255,(target_img_size,target_img_size))
			
		if x[0] in gen_list:
			# print("FOUND")
			gen_tag_img = cv2.imread(tag_dir_pathgen+x[0],0)
			ggpts,ggptsf = getFlippedZoomImages(gen_tag_img)
			gen_loc = cv2.resize(ggpts/255,(target_img_size,target_img_size))
			gen_locf = cv2.resize(ggptsf/255,(target_img_size,target_img_size))
		
		comb_feat = np.zeros((target_img_size,target_img_size,3),dtype=np.float32)
		comb_featf = np.zeros((target_img_size,target_img_size,3),dtype=np.float32)
		...
		blank_target = np.zeros((target_img_size,target_img_size,3),dtype=np.float32)
		# comb_feat = np.zeros((50,50,3),dtype=np.float32)
		# comb_feat[:,:,0]=fem_loc
		# comb_feat[:,:,1]=pel_loc
		# comb_feat[:,:,2]=gen_loc	
		# pos_img_300_flip = cv2.flip(pos_img_300,1)
		target_array = target_array + [comb_feat,comb_featf,blank_target,blank_target]
