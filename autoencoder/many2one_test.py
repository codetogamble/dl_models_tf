...
lstm_cell_2 = Layers.LSTM(32, activation='relu', return_sequences=False)(lstm_cell_1)
# lstm_3 = Layers.RepeatVector(batch_size)(lstm_cell_2)
# lstm_3 = tf.reshape(lstm_cell_2,(1,1,32))
...
for f in os.listdir(test_folder):
	...
	while(tt):
		...
		if len(img_array) == batch_size and len(img_array2) == batch_size:
			...
			print(c)
			c2 = sess.run(deconv6,feed_dict={input_image:img_array2})
			print(c2)
			cv2.imshow("IMG1,",c[-1])
			cv2.imshow("IMG2,",c2[-1])
			cv2.waitKey(0)
