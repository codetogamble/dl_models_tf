...
const_g = tf.graph_util.convert_variables_to_constants(sess,new_g,['conv2d_4/Relu'])
tf.reset_default_graph()
tf.graph_util.import_graph_def(const_g)
g = sess.graph
# print(subgraph)
# print(graphdef)
# exit()
...
for epoch in range(1):
	for x in training_set[:]:
		...
		if len(img_array)>=batch_size or training_set.index(x) == (len(training_set)-1):
			...
			output = sess.run(class_output,feed_dict={input_image:img_array})
			print(output)
