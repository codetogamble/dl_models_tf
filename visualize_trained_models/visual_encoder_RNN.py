...
conv_out = g.get_tensor_by_name('conv2d_4/Relu:0')
print(dir(conv_out))
...
for f in os.listdir(test_folder):
	...
	while(tt):
		...
		if sum_mid_vert < 2000 and sum_mid_hori > 5000:
			...
			img_array.append(out/255)
			img_array2.append(out2/255)
			if len(img_array) == 10 and len(img_array2) == 10:
			# output = sess.run(conv_out,feed_dict={input_image:[out/255,out2/255]})
				output = sess.run(conv_out,feed_dict={input_image:img_array})
				output2 = sess.run(conv_out,feed_dict={input_image:img_array2})
				img_array = []
				img_array2 = []
				print(output.shape)
				# exit()
